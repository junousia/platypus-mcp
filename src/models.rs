...
#[derive(Debug, Deserialize, JsonSchema)]
pub struct QuickCreateBacklogItemParams {
    /// Project root that bounds all file, Git, and state operations.
    pub root: Option<String>,
    /// Stable local record identifier. Usually omit this and let Platypus allocate
    /// the next ID from id_prefix.
    #[schemars(example = example_item_id(), pattern(r"^[A-Z]+-[0-9]{3}$"))]
    pub id: Option<String>,
    /// Identifier prefix used when allocating a new local backlog item ID when id
    /// is omitted.
    #[schemars(example = example_id_prefix(), pattern(r"^[A-Z]+$"))]
    pub id_prefix: Option<String>,
    #[serde(default)]
...
pub struct CreateBacklogItemsParams {
    ...
    #[serde(default)]
    /// Preview the batch without writing files. Preview still allocates the same
...
pub struct CreatedBacklogItemsData {
    /// Project root that bounds all file, Git, and state operations.
    pub root: String,
    /// Backlog items created or previewed by this batch.
...
pub struct CreatedBacklogBatchItem {
    ...
    /// Whether this tool call created the file, record, or workspace.
    pub created: bool,
    /// Normalized backlog item content generated by the canonical creation path.
    pub preview: CreatedBacklogItemPreview,
}

#[derive(Debug, Serialize, JsonSchema)]
pub struct CreatedBacklogItemPreview {
    /// Human-readable title or short label.
    pub title: String,
    /// Backlog priority.
    pub priority: String,
