...
export function shouldShowGuidance(prompt) {
	return /\b(backlog|platypus|platy|what\s+next|next\s+item|queue|status|complete|completion|plan|planning|story|stories|work\s+item|work\s+items)\b/i.test(prompt);
}
